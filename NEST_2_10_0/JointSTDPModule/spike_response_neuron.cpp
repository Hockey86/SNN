...
void spike_response_neuron::Parameters_::set(const DictionaryDatum &d, State_& s)
{
  ...
  if ( U_noise_ > 0 && noise_.empty() )
	...
	*/
  if ( U_epsp_ < 0 )
    throw nest::BadProperty( "EPSP cannot be negative." );
    
  if ( U_ipsp_ < 0 )
    throw nest::BadProperty( "IPSP cannot be negative. Please use its absolute value." );
...
void
spike_response_neuron::State_::get( DictionaryDatum& d ) const
{
  def< double >( d, nest::names::V_m, V_m_ ); // Membrane potential
}

void
spike_response_neuron::State_::set( DictionaryDatum const& d )
{
...
void
spike_response_neuron::init_node_( const nest::Node &proto )
{
...
void
spike_response_neuron::init_state_( const nest::Node &proto )
{
  const spike_response_neuron &pr = downcast< spike_response_neuron >( proto );
  S_ = pr.S_;
}
...
void
spike_response_neuron::init_buffers_()
{
...
void
spike_response_neuron::calibrate()
{
...
void
spike_response_neuron::update( const nest::Time& origin,
...
void
spike_response_neuron::handle( nest::SpikeEvent &e )
{
...
void
spike_response_neuron::handle( nest::DataLoggingRequest &e )
{
  B_.logger_.handle( e );
}
