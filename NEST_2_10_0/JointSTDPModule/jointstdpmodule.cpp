...
const std::string
mynest::JointSTDPModule::name( void ) const
{
 return std::string( "Joint STDP Module" ); // Return name of the module
}

const std::string
mynest::JointSTDPModule::commandstring( void ) const
{
...
void
mynest::JointSTDPModule::init( SLIInterpreter* i )
{
  /* Register a neuron or device model.
     Give node type as template argument and the name as second argument.
     The first argument is always a reference to the network.
  */
  nest::register_model< parrot_neuron2 >( nest::NestModule::get_network(), "parrot_neuron2" );
  nest::register_model< spike_response_neuron >( nest::NestModule::get_network(), "spike_response_neuron" );
  ...
  */
  nest::register_connection_model< WeightSTDPConnection< nest::TargetIdentifierPtrRport > >( nest::NestModule::get_network(), "weight_stdp_synapse" );
  nest::register_connection_model< JointSTDPConnection<nest::TargetIdentifierPtrRport > >( nest::NestModule::get_network(), "joint_stdp_synapse" );
  nest::register_connection_model< ImmediateInhibitionConnection< nest::TargetIdentifierPtrRport > >( nest::NestModule::get_network(), "immediate_inhibition_synapse" );
